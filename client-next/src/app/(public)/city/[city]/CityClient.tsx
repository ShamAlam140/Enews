...
export default function CityClient({ city, initialFiles }: { city: string; initialFiles: CityItem[] }) {
  ...
  return (
    <div className="mx-auto max-w-screen-xl px-3 md:px-6 py-6 md:py-10 text-gray-900">
      ...
      {isFilteredSingleFile && targetPageNum && (
        <div className="mb-6 rounded-xl border border-green-200 bg-green-50 p-4 text-sm text-green-900 flex flex-wrap items-center justify-between gap-3 shadow-xs">
          <div className="flex items-center gap-2">
            <span className="text-xl">📄</span>
            <div>
              <p className="font-bold text-green-950">
                आप {title} ई-पेपर का <span className="underline">पेज नंबर {targetPageNum}</span> देख रहे हैं
              </p>
              <p className="text-xs text-green-800 mt-0.5">
                शेयर किया गया पन्ना नीचे हाइलाइट किया गया है।
              </p>
            </div>
          </div>
          <button
                ...
                {slice.map((p) => {
                  ...
                  return (
                    <div
                      key={p.page}
                      id={`page-${f.id}-${p.page}`}
                      className={`relative border bg-gray-50 rounded-xl overflow-hidden w-full min-h-[400px] md:min-h-[600px] flex items-center justify-center transition-all duration-500 ${
                        isTargetPage ? "ring-4 ring-red-500 shadow-xl scroll-mt-28" : "hover:border-slate-300"
                      }`}
                    >
                      ...
                      {/* Page Badge (Top Left) */}
                      <span className="absolute top-3 left-3 text-xs bg-black/75 backdrop-blur text-white px-2.5 py-1 rounded-md font-bold z-10 shadow-sm">
                        Page {p.page}
                      </span>

                      {/* Action Buttons (Top Right: Download Page & WhatsApp Share) */}
                      <div className="absolute top-3 right-3 z-10 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 bg-slate-900/85 hover:bg-black backdrop-blur text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg transition transform hover:scale-105 active:scale-95 disabled:opacity-50"
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1.5 bg-[#25D366] hover:bg-[#20ba5a] text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg transition transform hover:scale-105 active:scale-95"
